...
export default function CommittePage() {
    const [committeMembers, setCommitteMembers] = useState<Director[]>([]);
    const [loading, setLoading] = useState(true);

    const fetchMembers = async () => {
        try {
            setLoading(true);
            const res: any = await axiosInstance.get("/committe");
            ...
            const baseUrl = process.env.NEXT_PUBLIC_ASSETS_URL || "https://assets.sanakisan.magnus.com.np";
            ...
            setCommitteMembers(mappedMembers);
        } catch (error) {
            console.error("Failed to fetch committe members:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const groupCommitteMembers = (members: Director[]) => {
    ...
    const groupedMembers = groupCommitteMembers(committeMembers);
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10 text-center">
                    <motion.div
                        ...
                        <h2 className="text-2xl md:text-[32px] font-bold tracking-tight text-slate-900">
                            Committe <span className="text-amber-600 relative">Members

...
export default function TeamsPage() {
    const [teamMembers, setTeamMembers] = useState<Director[]>([]);
    const [loading, setLoading] = useState(true);

    const fetchMembers = async () => {
        try {
            setLoading(true);
            const res: any = await axiosInstance.get("/team");
    ...
    const groupteamMembers = (members: Director[]) => {
    ...
    const groupedMembers = groupteamMembers(teamMembers);
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10 text-center">
                    <motion.div
                        ...
                        <h2 className="text-2xl md:text-[32px] font-bold tracking-tight text-slate-900">
                            Management <span className="text-amber-600 relative">Teams
